...
public partial class MainWindowViewModel : ViewModelBase
{
    ...
    private void RefreshList()
    {
        ...
        if (!string.IsNullOrWhiteSpace(FilterTagsText))
        {
            ...
            query = query.Where(item => 
                ...
                item.DisplayName.ToLower().Contains(FilterTagsText.ToLower())
            );
        }

        if (CurrentTab == "Task")
        {
            if (SelectedFilterPriority != "Wszystkie")
            {
        ...
        {
            foreach (var item in query) FilteredEntries.Add(item);
        }
    }
    ...
    private void UpdateExistingTags()
    {
        ...
        ExistingTags.Clear();
        foreach (var tag in distinctTags) ExistingTags.Add(tag);
    }
